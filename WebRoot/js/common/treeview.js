var TreeViewComponent = function (id, data, deep) {
    ...
    function create_tree_node(account, tree, deep) {
        ...
        var node = $("<li class='child_root_account'></li>");
        $(node).append("<div class='child_account no_child' accountId='{1}'><span title='{0}'>{0}({2}/{3})</span></div>".format(account.name, account.id, account.usedChildNum, account.childNum));
        if (account.attach.length > 0 && deep > 0) {
        	...
        	$(node).append("<span class='tree_no_click'></span>");
        }
    ...
    var Loader = {
        load_tree_component: function (id, data, deep) {
            ...
            $(tree).find("li:first .child_account:first")
            .removeClass("child_account")
            .addClass("root_account")
            .next(".tree_no_click").remove();
            $(id).append(tree);
        }
    }
    var Binder = {
    ...
    TreeViewComponent.prototype = {
        bind_child_node_click: function (callback) {
            $(".treeview .root_account").click(click_account_callback);
            $(".treeview .child_account").click(click_account_callback);
            
            function click_account_callback(){
            	$(".treeview .select_account").removeClass("select_account");
            	$(this).find("span").addClass("select_account");
            	callback(this);
            }
        }
    };
