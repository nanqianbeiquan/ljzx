...
public class AuthImageCodeUtil {

	private static AuthImageCodeUtil instance;
	
	private AuthImageCodeUtil(){}
	
	public static synchronized AuthImageCodeUtil getInstance(){
		
		if(instance==null){
			instance = new AuthImageCodeUtil();
		}
		return instance;
	}
	
	public static final int WIDTH = 120;//生成的图片的宽度
    public static final int HEIGHT = 42;//生成的图片的高度

    
    /**
     * 
     * @param bi 内存中的图片对象
     * @param typeFlag 验证码类型
     *          ch 生成中文验证码图片
     *          nl 生成数字和字母组合的验证码图片
     *          n 生成纯数字的验证码图片
     *          l 生成纯字母的验证码图片
     * @return
     */
    public String buildCode(BufferedImage bi,String typeFlag){
    	
    	if(typeFlag==null||"".equals(typeFlag.trim())){
    		typeFlag = "nl";
    	}
    	 //2.得到图片
        ...
        String random = drawRandomNum((Graphics2D) g,typeFlag);//根据客户端传递的createTypeFlag标识生成验证码图片
    ...
    private String drawRandomNum(Graphics2D g,String... createTypeFlag) {
        // 设置颜色
        g.setColor(new Color(230, 133, 5));
        // 设置字体
        ...
        //数字和字母的组合,去掉ILJ0OQ92Z
        String baseNumLetter = "345678ABCDEFGHKMNPRSTUVWXY";
        //纯数字
